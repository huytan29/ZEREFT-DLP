...
static int dbg_flag = 0;     /*debug flag - used while debug program*/
static int chroot_flag = 0;  /*root flag*/
static int daemon_flag = 0;  /*daemon flag - used while don't want to write log in terminal*/
static int pid;              /*pid of the calling process*/

/*Flag to set log format*/
static struct{
...
void SetNameModule(const char *name)
{
    __local_name = name;
    return;
}
//const char *__local_name = "LancsDLP";
...
/*
Check if the specified file is a regular file or a symbolic link to a regular file 
 Should be place in a file handle folder
*/
...
static void _log(int level, const char *tag, const char * file, int line, const char * func, const char *msg, va_list args)
{
    ...
    if (!flags.initialized) {
        logging_init();
        ldebug("Debug: Logging module auto-initialized");
    }

    if (filename = strrchr(file, '/'), filename) {
        file = filename + 1;
    }
    /*if log_json is set. We print log in json format*/
    if (flags.log_json) {
#ifndef WIN32
        int oldmask;
        ...
        if (!IsFile(logfile)) {
            ...
            // Make sure that the group is lancsdlp 
            // Will be developed when implement linux agent 
            /*
            if (fp && getuid() == 0) {
                gid_t group;
    ...
    if (flags.log_plain) {
      /* If under chroot, log directly to /logs/ossec.log */
        ...
        if (fp) {
            ...
            (void)fprintf(fp, "\n");
            fflush(fp); /*ensures that the data is physically written to the file and not just buffered in memory.*/
            w_mutex_unlock(&logging_mutex);
