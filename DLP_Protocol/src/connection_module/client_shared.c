/******************************************************************************
 * File:        client_shared.c 
 *
 * Description: Helper program used for all client MQTT program in DLP connection module
 *
 * Developer:      Daohs <daohs@lancsnet.com>
 *
 * Created:     1/10/2023
 *
 * Modified:    10/10/2023
...
int Read_Pub_Sub(XML_NODE node, void *d1, __attribute__((unused)) void *d2)
{
   ...
   const char *cafile = "cafile";
    unsigned int i = 0;
   struct mosq_config* mcfg = (struct mosq_config*)d1;
    while(node[i])
    {
        if(!node[i]->element)
        {
        ...
        {
            printf("Invalid NULL content\n");
            return (-1);
        }else if(strcmp(node[i]->element,topic) == 0)
        {
			cfg_add_topic(mcfg,CLIENT_SUB, strdup(node[i]->content),"-t");
            printf("pub topic : %s\n",node[i]->content);
        }else if(strcmp(node[i]->element,qos) == 0)
        {
            mcfg->qos = atoi(strdup(node[i]->content));
            printf("pub qos : %s\n",node[i]->content);
        }else if(strcmp(node[i]->element,keepalive) == 0)
        {
            mcfg->keepalive = atoi(strdup(node[i]->content));
            printf("pub keepalive : %s\n",node[i]->content);
        }else if(strcmp(node[i]->element,cafile) == 0)
        {
            mcfg->cafile = strdup(node[i]->content);
            printf("pub cafile : %s\n",node[i]->content);
        }else{
            printf("Invalid element configuration: %s\n",node[i]->element);
            return (-1);
        }
        i++;
    }
...
{
    int i = 0; //counter to iterate through 
    /*Define main element of config file*/
    ...
    const char *pub = "pub";
    const char *sub = "sub";
    struct mosq_config* cfg = (struct mosq_config*)d1;
    //memset(mcfg,0,sizeof(mcfg));
	init_config(cfg,CLIENT_SUB);
    while(node[i])//start from node 0
    {
        ...
        if((strcmp(node[i]->element,host)) == 0){
            cfg->host = strdup(node[i]->content);
            printf("host : %s\n",node[i]->content);
        }
        else if((strcmp(node[i]->element,port)) == 0){
            cfg->port = atoi(strdup(node[i]->content));
            printf("port : %s\n",node[i]->content);
        }else if((strcmp(node[i]->element,username)) == 0){
            cfg->username = strdup(node[i]->content);
            printf("username : %s\n",node[i]->content);
        }else if((strcmp(node[i]->element,agent_id)) == 0){
            cfg->id = strdup(node[i]->content);
            printf("agent_id : %s\n",node[i]->content);
        }
        else if((strcmp(node[i]->element,password)) == 0){
            cfg->password = strdup(node[i]->content);
            printf("password : %s\n",node[i]->content);
        }
        else if(child_node && (strcmp(node[i]->element,pub) == 0))
        {
            if((modules & PUB) && Read_Pub_Sub(child_node,d1,d2) < 0 )
            {
                goto fail;
            }
        }
        else if(child_node && (strcmp(node[i]->element,sub) == 0))
        {
            if((modules & SUB) && Read_Pub_Sub(child_node,d1,d2) < 0 )
            {
                goto fail;
            }
        }else{
            printf("Invalid configuration element: %s\n",node[i]->element);
            goto fail;
        }
...
#endif
*/	//loc = strdup("/home/lancs/.config/mosquitto_sub");
	if(loc){
		fptr = fopen(loc, "rt");
		if(fptr){
			while(fgets(line, 1024, fptr)){
				if(line[0] == '#') continue; /* Enconter Comments jump to next line */

				while(line[strlen(line)-1] == 10 || line[strlen(line)-1] == 13){ //remove "\n"
			...
			ldebug("Error %s: %s",strerror(errno),loc);//debug
		}
		free(loc);
	}
