 ...
 * File:        sub_client.c 
 *
 * Description: Main program for subscribe client MQTT in DLP connection module
 *
...
int main(int argc, char *argv[])
{
...
#endif
	/*Set name for program, needed for logging*/
	SetNameModule("DLP_Sub");
	mosquitto_lib_init();
	output_init();
	nowDebug();/*For debugging purpose*/
	const char *cfgfile = "/home/lancs/DataLossPrevention/codebase/LancsDLP/LancsDLP/lancs_dlp.conf";
	//rc = ReadConfig(SUB,cfgfile,&cfg,NULL);
	//rc = client_config_load(&cfg, CLIENT_SUB, argc, argv);
	int modules = 0;
    modules |= SUB;
    if(ReadConfig(modules,cfgfile,&cfg,NULL) < 0 )
    {
        printf("fail to readconfig\n");
    }
    printf("cfg->username: %s\n",cfg.username);
    printf("cfg->host: %s\n",cfg.host);
    printf("cfg->port: %d\n",cfg.port);
    printf("cfg->topic: %s\n",cfg.topic);
    printf("cfg->keepalive: %d\n",cfg.keepalive);
	printf("cfg>keepalive: %d\n",cfg.random_filter);
	printf("Mqtt ver:%d\n",cfg.protocol_version);
	printf("Num of topics: %d\n",cfg.topic_count);
	cfg.debug = true;
	// if(rc){
	// 	if(rc == 2){
	// 		/* --help */
	...
	linfo("Create a new instance successfuly");
