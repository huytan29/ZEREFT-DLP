/******************************************************************************
 * File:        sub_client_output.c 
 *
 * Description: Helper program for subscribe client MQTT in DLP connection module
 *
...
void print_message(struct mosq_config *lcfg, const struct mosquitto_message *message, const mosquitto_property *properties)
{
	...
	if(root)
	{
		cJSON *usb = cJSON_GetObjectItem(root, "USB");
		cJSON *hdd = cJSON_GetObjectItem(root, "HDD");
		printf("USB state: %s\n",usb->valuestring);
		printf("USB state: %s\n",hdd->valuestring);
		cJSON_Delete(root);
	}else{
		ldebug("Cannot parse json");
	}
#ifdef WIN32
	unsigned int r = 0;
#else
